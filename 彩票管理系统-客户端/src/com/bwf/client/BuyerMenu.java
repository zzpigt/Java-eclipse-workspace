...
public class BuyerMenu {
	...
	public BuyerMenu(ObjectOutputStream oos,MyResponse lastResponse) {
		this.oos = oos;
		this.lastResponse = lastResponse;
	}
	
	public void buyerMenu(String uName) throws IOException {
	...
	private boolean buyerLogout(String uName) throws IOException {
		...
		if ("y".equals(chose) || "Y".equals(chose)) {
			...
			MyRequest request = new MyRequest(MyRequest.TYPE_USER_LOGOUT);
			request.getUmap().put(MyRequest.MEG_USERNAME, uName);
			request.getUmap().put(MyRequest.MEG_LEVEL, "彩民");
			// 包装好了，就发送过去，序列化
			oos.writeObject(request);
	...
	private void bet(String uName) throws IOException {
		...
		do {
			...
			if(lastResponse.isIssue()) {
				...
				ltime = lastResponse.getrMeg().get(MyResponse.LETTERY_TIME);
				...
				System.out.println("显示关彩票信息:");
				System.out.println(lastResponse.getrMeg().get(MyResponse.LETTERY_CONTENT));
				continue;
			}

			System.out.println("刷新：");
			String flush = ClientDemo.sc.next();
			MyRequest request_flush = new MyRequest(MyRequest.TYPE_BET_FLUSH);
			request_flush.getUmap().put(MyRequest.MEG_USERNAME, uName);
			request_flush.getUmap().put(MyRequest.MEG_LEVEL, "彩民");
			request_flush.getUmap().put(MyRequest.LOTTERY_TIME, ltime);
			// 包装好了，就发送过去，序列化
			oos.writeObject(request_flush);
			oos.flush();
			...
			synchronized (ClientDemo.class) {
				...
				//这里是拿到了购买者历史记录
				System.out.println(lastResponse.getHistoryList());
				List<String> historyList = lastResponse.getHistoryList();
